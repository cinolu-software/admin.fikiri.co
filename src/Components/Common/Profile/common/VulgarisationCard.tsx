...
const VulgarisationCard: React.FC<{user : UserProfileType }> = (user) => {
    const [copied, setCopied] = useState(false);

    const handleCopy = () => {
        navigator.clipboard.writeText(`https://fikiri.co/sign-up?link=${user.user?.popularization_link || ''}`);
        setCopied(true);
        setTimeout(() => setCopied(false), 2000);
    };

    if (!user.user?.popularization_link) {
        return null;
    }
        ...
        <div className="mt-5 mb-5 border rounded p-4 bg-light-primary text-center">
            ...
            <div className="input-group mb-3 position-relative">
                <input
                    ...
                    value={`https://fikiri.co/sign-up?link=${user.user?.popularization_link}`}
                    readOnly
